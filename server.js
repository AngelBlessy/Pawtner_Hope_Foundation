...
// Utility functions for responses file operations
const readResponsesFromFile = () => {
    try {
        const data = fs.readFileSync(RESPONSES_FILE, 'utf8');
        return JSON.parse(data);
    } catch (error) {
        console.error('Error reading responses file:', error);
        return { services: [], service_requests: [], service_categories: [] };
    }
};

const writeResponsesToFile = (data) => {
    try {
...
app.get('/api', (req, res) => {
    res.json({
        ...
        endpoints: {
            ...
            'GET /api/service-requests': 'Get all service requests from forms',
...
app.get('/api/service-requests', (req, res) => {
    try {
        const data = readResponsesFromFile();
        let serviceRequests = data.service_requests || [];

        // Apply filters
        const { _limit, ...filters } = req.query;
        if (Object.keys(filters).length > 0) {
            serviceRequests = filterData(serviceRequests, filters);
        }

        // Apply limit
        if (_limit) {
            serviceRequests = serviceRequests.slice(0, parseInt(_limit));
        }
...
app.get('/api/service-requests/:id', (req, res) => {
    try {
        const data = readResponsesFromFile();
        const serviceRequest = data.service_requests.find(sr => sr.id === parseInt(req.params.id));
...
app.post('/api/service-requests', (req, res) => {
    try {
        const { name, email, phone, services, adoptionPreference, preferredDate, preferredTime, contactMethod } = req.body;

        // Validation
        if (!name || !email || !phone || !preferredDate || !preferredTime || !contactMethod) {
            return res.status(400).json({
                success: false,
                message: 'Missing required fields: name, email, phone, preferredDate, preferredTime, contactMethod'
            });
        }

        const data = readResponsesFromFile();
        const newServiceRequest = {
            id: generateId(data.service_requests),
            name,
            email,
            phone,
            services: services || [],
            adoptionPreference: adoptionPreference || 'Any',
            preferredDate,
            preferredTime,
            contactMethod,
            status: 'Pending',
        ...
        data.service_requests.push(newServiceRequest);
        
        if (writeResponsesToFile(data)) {
            res.status(201).json({
                success: true,
...
app.put('/api/service-requests/:id', (req, res) => {
    try {
        ...
        const data = readResponsesFromFile();
        const serviceRequestIndex = data.service_requests.findIndex(sr => sr.id === serviceRequestId);
        ...
        // Update service request with new data
        data.service_requests[serviceRequestIndex] = { ...data.service_requests[serviceRequestIndex], ...updates, id: serviceRequestId };

        if (writeResponsesToFile(data)) {
            res.json({
                success: true,
                message: 'Service request updated successfully',
                data: data.service_requests[serviceRequestIndex]
            });
        } else {
            throw new Error('Failed to save data');
        }
...
app.delete('/api/service-requests/:id', (req, res) => {
    try {
        ...
        const deletedServiceRequest = data.service_requests.splice(serviceRequestIndex, 1)[0];
